...
export default function ChatMessage({ sender, message }) {
  ...
  useEffect(() => {
    if (!containerRef.current) return; // <-- add this guard
  ...
  rounded-2xl rounded-bl-none whitespace-pre-wrap break-words
  ...
  prose-ul:my-0 prose-ol:my-0 prose-li:my-0 prose-li:p-0
  prose-li:leading-normal
  prose-li > p:my-1 prose-li > p:leading-relaxed
  ...
  cursor-text
  target-blank:prose-a:text-orange-600 
  target-blank:prose-a:no-underline hover:target-blank:prose-a:underline
